...
class DialogGeometry(object):
    ...
    def from_dict(data: Dict[str, int]):
        return DialogGeometry(
            position=QPoint(data[DialogGeometry._KEY_X], data[DialogGeometry._KEY_Y]),
            size=QSize(data[DialogGeometry._KEY_WIDTH], data[DialogGeometry._KEY_HEIGHT])
        )

    def to_dict(self) -> Dict[str, int]:
        return {
            DialogGeometry._KEY_X: self.position.x(),
            DialogGeometry._KEY_Y: self.position.y(),
...
class UICache(object):
    ...
    def __init__(self, workspace_path: str):
        ...
        self.__dialog_geometry_cache: Dict[UUID, DialogGeometry] = dict()

    def update_dialog_geometry(self, dialog_id: UUID, position: QPoint, size: QSize):
        self.__dialog_geometry_cache[dialog_id] = DialogGeometry(position=position, size=size)

    def get_dialog_geometry(self, dialog_id: UUID) -> Union[Tuple[QPoint, QSize], None]:
        if dialog_id in self.__dialog_geometry_cache:
            record = self.__dialog_geometry_cache[dialog_id]
            return record.position, record.size
        ...
        if os.path.isfile(dialog_geometry_cache_file_path):
            with open(dialog_geometry_cache_file_path, 'r') as fp:
                ...
                for key in dialog_geometry_cache:
                    ...
                    self.__dialog_geometry_cache[UUID(key)] = DialogGeometry.from_dict(value)
    ...
    def save(self):
        ...
        with open(dialog_geometry_cache_file_path, 'w') as fp:
            ...
            for dialog_geometry_key in self.__dialog_geometry_cache:
                ...
                dialog_geometry_records[dialog_geometry_key.__str__()] = dialog_geometry_value.to_dict()
