...
class CreationTemplateEditorView(object):
    ...
    def __init__(self, dialog: QDialog, vm: CreationTemplateEditorViewModel):
        ...
        self.name_title = create_row_title(r'模板名称')
        self.name_input = QLineEdit()
        self.name_layout = create_row_layout(title=self.name_title, widget=self.name_input)

        css = '''font-family: 'Consolas';
        font-size: 12px;'''

        self.content_title = create_row_title(r'模板内容')
        self.content_input = QPlainTextEdit()
        self.content_input.setStyleSheet(css)
        self.content_layout = create_row_layout(title=self.content_title, widget=self.content_input)
        self.content_layout.setAlignment(self.content_title, Qt.AlignmentFlag.AlignTop)

        self.parameter_title = create_row_title(r'参数列表')
        ...
        self.parameter_layout = create_row_layout(title=self.parameter_title, widget=self.parameter_input)
