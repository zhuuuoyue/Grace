...
def calculate_angle(first_line: Line, second_line: Line) -> Tuple[bool, float, str]:
    ...
    """
    if first_line.start_point.distance(first_line.end_point) == 0:
        return False, 0, '第一条线段长度为 0'
    if second_line.start_point.distance(second_line.end_point) == 0:
...
class SpatialRelationshipOfTwoLinesModel(WidgetModelBase):
    ...
    tolerance_changed = Signal(type(float))
    ...
    tolerance = Property(type(float), fget=get_tolerance, fset=set_tolerance, notify=tolerance_changed)
    ...
    def __on_angle_changed(self, angle: Union[float, None]):
        if angle is None:
            ...
            self.conclusion = infer_spatial_relationship(angle, self.get_tolerance())
...
class SpatialRelationshipOfTwoLinesViewModel(WidgetViewModelBase):
    ...
    def __on_model_angle_changed(self, angle: Union[float, None]):
        self.angle_text = str() if angle is None else f'{angle} degree'
    ...
    def __on_model_conclusion_changed(self, conclusion: SpatialRelationship):
        self.conclusion_text = {
            ...
            SpatialRelationship.PARALLEL: '平行',
            SpatialRelationship.PERPENDICULAR: '垂直',
        }[conclusion]
...
class SpatialRelationshipOfTwoLinesView(object):
    ...
    def __init__(self, dialog: QDialog):
        ...
        self.tolerance_layout = create_row_layout(title=self.tolerance_title, widget=self.tolerance_slider)
...
class SpatialRelationshipOfTwoLinesDialog(DialogBase):
    ...
    def initialize(self):
        ...
        self.ui.second_line_input.line_changed.connect(self.__on_second_line_changed)
    ...
    @Slot(type(Union[Line, None]))
    def __on_first_line_changed(self, line: Union[Line, None]):
    ...
    @Slot(type(Union[Line, None]))
    def __on_second_line_changed(self, line: Union[Line, None]):
